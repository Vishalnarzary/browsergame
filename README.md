...
A Gemini 2.5 Flash request starts immediately with every run and repeats at 30-second intervals, returning exactly seven funny office sentences with at least one motivational quote. The queue advances independently every five seconds: each line appears in a guaranteed readable office caption and is also attached to a farther employee when one is available. A persistent device history, client filtering, server validation, and explicit prompt context prevent old or within-batch lines from repeating; unused unique fallbacks preserve the feature whenever the API is unavailable.
